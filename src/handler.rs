...
fn handle_popup_input(app: &mut App, key_event: KeyEvent, popup: Popups) {
    match popup {
        ...
        Popups::ColorSelection => match key_event.code {
            ...
            KeyCode::Char(' ') | KeyCode::Enter => app.color_selected(),
            _ => fallback_key_handler(app, key_event),
        },
...
fn chess_inputs(app: &mut App, key_event: KeyEvent) {
    ...
    match key_event.code {
        ...
        KeyCode::Right | KeyCode::Char('j' ) => match app.game.game_state {
            GameState::Promotion => app.game.ui.cursor_right_promotion(),GameState::Playing => app.go_right_in_game(),
            _=> (),
        },

        KeyCode::Left | KeyCode::Char('l') => match app.game.game_state {
            GameState::Promotion => app.game.ui.cursor_left_promotion(), GameState::Playing => app.go_left_in_game(),
            _=> (),
        },
    ...
    match key_event.code {
        KeyCode::Char('b') => {
            ...
            if let Some(opponent) = app.game.opponent.as_mut() {
                opponent.send_end_game_to_serve();
                app.hosting = None;
                app.host_ip = None;
            }
...
pub fn handle_mouse_events(mouse_event: MouseEvent, app: &mut App) -> AppResult<()> {
    ...
    if mouse_event.kind == MouseEventKind::Down(MouseButton::Left) {
        ...
        let authorized_positions = app
            .game.game_board.get_authorized_positions(app.game.player_turn, app.game.ui.selected_coordinates);


        let piece_color = app.game.game_board.get_piece_color(&app.game.ui.selected_coordinates);


        if authorized_positions.contains(&coords) && match piece_color {
