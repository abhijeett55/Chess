...
impl App {
    ...
    pub fn setup_game_server(&mut self, host_color: PieceColor) {
        ...
        log::info!("Starting game_server with host color: {:?} ", host_color);
    ...
    pub fn create_opponent(&mut self) {
        ...
        if self.hosting.unwrap() {
            ...
            self.host_ip = Some(format!("{}: 3600", self.get_host_ip()));
        }
        ...
        if !self.hosting.unwrap() {
            log::info!("Setting up clients (non-host) player");
    ...
    pub fn color_selected(&mut self) {
        self.current_popup = None;
        let color = match self.menu_cursor {
            0 => PieceColor::White,
            1 => PieceColor::Black,
            _ => unreachable!("Invalid color selections"),
        };
    ...
    pub fn update_config(&mut self) {
        ...
        if let Some(table) = config.as_table_mut() {
            table.insert("display_mode".to_string(),
                         Value::String(self.game.ui.display_mode.to_string()),);
            table.insert("log_level".to_string(),
    ...
    pub fn go_left_in_game(&mut self)  {
        let authorized_positions = self
            .game
            .game_board
            .get_authorized_positions(self.game.player_turn,
