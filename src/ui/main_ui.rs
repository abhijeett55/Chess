...
use super::popups::{render_enter_multiplayer_ip, render_multiplayer_selection_popup,
                render_wait_for_other_payer,    };
...
pub fn render(app: &mut App, frame: &mut Frame<'_>){
        ...
        if app.hosting.is_none() {
            ...
            if app.host_ip.is_none() {
                if app.hosting.is_some() && app.hosting.unwrap() {
                    ...
                    app.host_ip = Some("123.0.0.1".to_string());
                } else {
    ...
    match app.current_popup {
        ...
        Some(Popups::WaitingForOpponentToJoin) => {
            render_wait_for_other_payer(frame , app.get_host_ip());
        }
...
pub fn render_game_ui(frame: &mut Frame<'_>, app: &mut App, main_area : Rect) {
    ...
    if app.game.game_state == GameState::Draw {
        render_end_popup(frame, "That's as draw", app.game.opponent.is_some());
    }
