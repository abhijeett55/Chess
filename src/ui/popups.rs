...
pub fn render_engine_path_error_popup(frame: &mut Frame) {
    ...
    let text = vec![
        ...
        Line::from(""),
        Line::from("To  do so use -e argument when running chess to store a engine path"),
        Line::from(""),
        Line::from("Example: "),
        Line::from("Chess "),
    ];
...
pub fn render_end_popup(frame: &mut Frame, sentence: &str, is_multiplayer: bool) {
    ...
    let text = vec![
        ...
        Line::from(if is_multiplayer {
            "Press 'B' to gop back to the game"
        } else {
...
pub fn render_promotion_popup(frame: &mut Frame, app: &mut App) {
    ...
    let text = vec![
        Line::from(""),
        Line::from("-- Center your pawn promotion --").alignment(Alignment::Center),
        Line::from(""),
    ];
...
pub fn render_credit_popup(frame: &mut Frame) {
    ...
    let credits_text = vec![
        Line::from(""),
        Line::from("Hi 👋, I'm Abhijeet, a 22 years old Abhijeet computer science student."),
        Line::from("Thank you for playing Chess! This project started as a personal journey to improve my algorithmic skills and learn Rust."),
        Line::from(""),
        Line::from("The entire source code is available on GitHub at "),
...
pub fn render_color_selection_popup(frame: &mut Frame, app: &App) {
    ...
    let text = vec![
        Line::from(""),
        Line::from("__ choose your color --").alignment(Alignment::Center),
        Line::from(""),
    ];
...
pub fn render_wait_for_other_payer(frame: &mut Frame, ip: IpAddr) {
    let block = Block::default()
    .title("Waiting...")
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
